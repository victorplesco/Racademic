...
#' 
#' 
v2_get_object_fields <- function(show.objects = FALSE, v2.object = NULL, read.only = FALSE, custom.fields = NULL, open.url = FALSE) {
  
  #' @description: to allow communication with main functions, @custom.fields accepts "all" as an option, alongside NULL, for requesting 
  #' all the fields of a specified @v2.object. To simplify code it was decided to switch @custom.param to NULL whenever "all" is requested.
  #' 
  #' 
  if(length(which(custom.fields %in% "all" == TRUE)) == length(custom.fields)) {custom.fields = NULL};
  ...
  #' @description: building dependencies of @open.url and @read.only with @v2.object. If @v2.object is NULL, throws an error, else, 
  #' depending on the value of @read.only, will return (if FALSE) or print (if TRUE) the fields of the specified @v2.object. It is 
  #' possible to customize the return, or print, by providing a set of fields through @custom.fields. Also if @open.url is TRUE, the 
  #' `browseURL()` function will be activated and the documentation page related to the provided @v2.object will be opened in your 
  ...
  if(!is.null(v2.object)) { # if an object is provided;
    ...
    if(v2.object %in% names(v2.objects_list) == FALSE) { # If no match with v2.object;
      stop("INPUT ERROR: the object provided is wrong!");
    }
    else { # If match with v2.object;
      
      if(open.url == TRUE) { # If open.url is TRUE;
...
}; # v2_get_object_fields(v2.object = "tweet.fields", read.only = FALSE, custom.fields = c("all"), open.url = FALSE);
